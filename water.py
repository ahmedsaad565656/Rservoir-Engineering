...
def calculate_water_formation_volume_factor_free():
    ...
    P = st.number_input("Enter the pressure ", min_value=0.0)
...
def calculate_water_Isothermal_Compressibility():
    st.title("calculate water Isothermal Compressibility")
    ...
    if st.button("Calculate Cw "):
        ...
        st.success(f"water Isothermal Compressibility  : {Cw:.7f} psi**-1")
#..
def calculate_Gas_Solubility_in_Water():
    st.title("calculate Gas Solubility in Water")
    P = st.number_input("Enter the pressure (psia) ", min_value=0.0)
    T = st.number_input("Enter the temperature (F)", min_value=0.0)
...
def calculate_water_viscosity_Brill():
    st.title("calculate water viscosity using Brill method")
...
def calculate_water_viscosity_Meehan():
    st.title("calculate water viscosity using Meehan method")
    P = st.number_input("Enter the pressure (psia)", min_value=0.0)
    T = st.number_input("Enter the temperature (R)", min_value=0.0)
...
options = {
    ...
    'Calculate Water formation volume factor(gas free water) ': calculate_water_formation_volume_factor_free,
    'Calculate Water formation volume factor(gas saturated water) ': calculate_water_formation_volume_factor_saturated,
    'Calculate Water Isothermal Compressibility ': calculate_water_Isothermal_Compressibility,
    'Calculate Gas Solubility in Water ': calculate_Gas_Solubility_in_Water,
    'Calculate water viscosity Brill ': calculate_water_viscosity_Brill,
    'Calculate water viscosity Meehan': calculate_water_viscosity_Meehan
